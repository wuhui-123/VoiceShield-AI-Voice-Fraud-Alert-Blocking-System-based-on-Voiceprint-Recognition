...
class FileInfo():
    ...
    expire_interval: float = 300

class TempFileRegistery():
    """
        文件注册表
    ...
    def __init__(self, auto_cleanup_expire_interval: int = 300, default_path: str = './temp_files') -> None:
        """
            初始化
        ...
        self._iter_index = 0                  # 迭代索引
        self._iter_files = []                 # 迭代文件列表
    ...
    def __iter__(self):
        self._iter_files = list(self.files.values())
        self._iter_index = 0
        return self

    def __next__(self):
        if self._iter_index >= len(self._iter_files):
            raise StopIteration
        item = self._iter_files[self._iter_index]
        self._iter_index += 1
        return item
